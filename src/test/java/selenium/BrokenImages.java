...
public class BrokenImages {
	...
	public static void main(String[] args) throws IOException {
		...
		int len = images.size();
		System.out.println(" Total number of Images in the application is " + len);
		
		for (int i=0;i<len;i++)
		{
			String urlfromwebsite = images.get(i).getAttribute("src");
			//System.out.println(urlfromwebsite);
			BrokenImages.validatation(urlfromwebsite);
		}
		
	}
		
		public static void validatation(String link) throws IOException
		{
		...
		{
			System.out.println(link + "-->" + http.getResponseMessage()+ " is a broken Image");
		}

	  else { 
		  System.out.println(link + "-->" + http.getResponseMessage()+" is not a Broken Image"); 
	  }
